...
CHALL_URL = "http://challenge:1337/login"
...
if not res.text.startswith("cr3{1") or not res.text.endswith("t0o}"):
    exit(1)

print('flag:', res.text)

payload = '{"admin":false,"checksum":'
s = encrypt_str(payload.encode())
key = mmh3_32(s, 0)
hilo = mmh3_64(s, 0)
payload += str(calc_checksum(hilo >> 32, hilo & 0xffffffff, key)) + "}"
...
if not res.text.startswith("cr3{w") or not res.text.endswith("49}"):
    exit(1)

print('fake flag:', res.text)
exit(0)
