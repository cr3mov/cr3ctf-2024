...
class Breaker():
    ...
    def ut(self, num):
        ...
        def set_bit_to_one(number, position):
            return number | BitVecVal((1 << (self.num_bits - 1 - position)), self.num_bits)

        def undo_right_shift_xor_and(result, shift_len, andd=-1):
            original = 0
            for i in range(self.num_bits):
                original = If(
                    (get_bit(result, i) ^ (get_bit(original, i - shift_len) &
                         get_bit(andd, i))) != 0,
                    set_bit_to_one(original, i),
                    original
                )
                # if get_bit(result, i) ^ \
        ...
        def undo_left_shift_xor_and(result, shift_len, andd):
            ...
            for i in range(self.num_bits):
                original = If(
                    (get_bit(result, self.num_bits - 1 - i) ^ (get_bit(original, self.num_bits - 1 - (i - shift_len)) &
                         get_bit(andd, self.num_bits - 1 - i))) != 0,
                    set_bit_to_one(original, self.num_bits - 1 - i),
                    original
                )
    ...
    def get_seed_mt(self, S, outputs):
        """
